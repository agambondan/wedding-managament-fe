        ...
        <input
			...
			placeholder={
				props.placeholder !== undefined
					? props.placeholder
					: (props.keyInput.match(/[a-zA-Z0-9]+/g) || [])
...
export function InputDate(props) {
	return (
		<input
			...
			autoComplete=''
			maxLength='10'
			className='px-3 py-3 my-1 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm border border-blueGray-300 outline-none focus:outline-none focus:ring w-full'
		/>
	);
}
...
export function InputNumber(props) {
	return (
        <input
			...
			name={props.keyInput}
			placeholder={(props.keyInput.match(/[a-zA-Z0-9]+/g) || [])
				.map((w) => `${w.charAt(0).toLowerCase()}${w.slice(1)}`)
				.join(' ')}
			value={props.inputFields}
