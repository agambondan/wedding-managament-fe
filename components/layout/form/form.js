...
export function Form(props) {
	...
	const handleFormatRupiah = (key, event) => {
		props.setInputFields({
			...props.inputFields,
			[key]: formatRupiah(event.target.value, ''),
		});
	};
		...
		<div className={'bg-gray-200 px-6 py-4'}>
			{props.default === undefined || props.default === true ? (
				<form onSubmit={handleSubmit}>
					...
					<button
						type={'submit'}
						className='mt-5 px-10 py-3 text-blue-100 transition-colors
                         duration-150 bg-blue-600 rounded-lg focus:shadow-outline hover:bg-blue-700'
					>
						Save
					</button>
				</form>
