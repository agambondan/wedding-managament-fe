...
import { GoogleMapEmbed } from '../../lib/maps';
			...
			<div className='lg:flex shadow rounded-l-lg border border-gray-400'>
				...
				<div className='w-full  lg:w-11/12 xl:w-full px-1 bg-white py-5 lg:px-2 lg:py-2 tracking-wide'>
					...
					<div className={'text-center lg:text-left lg:pt-0 pt-3'}>
						<span
							className='tracking-wider text-gray-600 bg-gray-200 px-2 text-sm rounded
				...
				<div
					className='flex lg:flex-col flex-row lg:items-end items-center w-full lg:w-1/3 bg-white
                    lg:justify-end justify-center px-2 lg:py-2 pb-4 lg:px-0'
				>
					<button
						className='tracking-wider text-white text-opacity-80 bg-blue-500
					...
					<button
						className='tracking-wider text-white text-opacity-80 bg-red-600
...
Agenda.layout = ClientLayout;

function FormModalAgenda(props) {
	...
	useEffect(() => {
		const fetchExpandedUrl = async () => {
			console.log(props.inputFields.link_location);
			try {
				// Fetch the full URL using the API route
				const response = await fetch(
					`/api/get-full-url?shortLink=${encodeURIComponent(
						props.inputFields.link_location
					)}`
				);
				const data = await response.json();
				if (data.fullUrl) {
					setEmbedUrl(data.fullUrl);
					setLat(data.lat);
					setLng(data.lng);
				} else {
					console.error('Failed to resolve the short link:');
				}
			} catch (error) {
		...
		if (props.inputFields.link_location) {
			fetchExpandedUrl();
		}
	}, [props.inputFields.link_location]);
	...
	useEffect(() => {
		async function fetch() {
			...
			request.url = `cities?filters=["id","${props.inputFields.id}"]&size=${res.data.total}&sort=code,name,${sort}&fields=id,name`;
	...
	return (
		<BaseModal title={props.title} setClick={props.setClick}>
			...
			<div className='flex flex-wrap -mx-3 py-2'>
				...
				<div className='w-full md:w-1/2 px-3'>
					{props.inputFields.id !== undefined &&
					props.inputFields.id !== null &&
					city[0].id !== '' ? (
						<Select
							data={city}
							setInputFields={props.setInputFields}
